...
async def send_main_menu(
    target_event: Union[types.Message,
    types.CallbackQuery],
    settings: Settings,
    i18n_data: dict,
    ...
    if settings.TRIAL_ENABLED:
        if hasattr(
                subscription_service, 'has_had_any_subscription',
        ) and callable(
    ...
    try:
        ...
        logging.warning(
            f"Failed to send/edit main menu (user: {user_id}, is_edit: {is_edit}): {type(e_send_edit).__name__} - "
            f"{e_send_edit}.",
        )
        if is_edit and target_message_obj and hasattr(
                target_message_obj, 'chat',
...
async def select_language_callback_handler(
    ...
    try:
        updated = await user_dal.update_user_language(
            session, user_id,
            lang_code,
        )
        if updated:
